...
- 普通用户通过登录获取 2 小时 Access Token 和 7 天 Refresh Token；Refresh Token 无效、类型错误或用户已禁用时统一返回 401。
- `/api/user/**`、购物车、订单、商品管理接口需要 JWT。
- Agent 调订单内部接口使用共享密钥请求头：`X-Internal-Service: agent` 与 `X-Internal-Secret`，后端校验后只返回订单摘要。

## 事务与并发

外层协调器会为每次下单尝试创建独立的 `REQUIRES_NEW` 事务；仅 version CAS 冲突允许在新快照中重试。扣库存 SQL 为：
...
影响行数为 0 时抛出 version 冲突并回滚该次尝试；重新读取后若库存不足则返回库存不足，重试耗尽且库存仍在时返回 409，避免超卖、少卖和脏订单。

## 本地上传与配置

- 管理员通过 `POST /api/products/upload-image` 上传 JPG、PNG、GIF 或 WEBP 图片（最大 5MB）；文件保存到后端 `/app/uploads`，由 `/uploads/**` 公开只读映射，Compose 使用 `backend_uploads` volume 持久化，Nginx 将该路径代理给后端。
- `MYSQL_ROOT_PASSWORD`、`JWT_SECRET` 和 `INTERNAL_SERVICE_SECRET` 是 Compose 必填变量；`JWT_SECRET` 至少32个字符。仓库不再提供可运行的默认密钥，启动前必须填写未提交的 `.env`。

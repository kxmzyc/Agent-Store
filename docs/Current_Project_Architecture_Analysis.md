# Agent-Store Current Project Architecture Analysis
...
Agent-Store 是一个“淘宝 + AI 导购助手”风格的智能商城实训项目。项目采用前后端分离、主业务后端与 Agent 微服务分离的架构，目标是在传统电商链路之外增加 AI 导购、长期偏好记忆、个性化推荐、用户行为采集和后台运营管理能力。

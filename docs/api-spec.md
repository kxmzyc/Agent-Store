...
- `POST /api/products`，管理员
  - 可选追加：`tagIds: [1, 3]`
- `POST /api/products/upload-image`，管理员，`multipart/form-data`
  - 字段：`file`
  - 限制：仅 JPG、PNG、GIF、WEBP，最大 5MB
  - 响应：`201 { "imageUrl": "/uploads/<uuid>.png" }`
  - 上传结果可通过 `GET /uploads/<filename>` 访问；文件持久化到 Compose 的 `backend_uploads` volume。

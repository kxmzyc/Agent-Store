...
一个面向企业实训答辩的“淘宝 + AI 助手”迷你商城系统。项目采用前后端分离架构，并将 AI 导购助手拆成独立微服务，覆盖用户、商品、搜索、购物车、订单和智能推荐助手的完整演示链路。

> 项目定位：教学实训项目，重点是功能闭环、工程结构、Docker 部署和 Agent 工具调用，不追求生产级高并发架构。

## 功能概览

- 用户与认证：注册、登录、JWT 鉴权、Token 刷新、个人中心、账户概览。
- 商品与搜索：分类浏览、商品列表、商品详情、关键词搜索、排序分页、管理员商品管理。
- 购物车与订单：加入购物车、数量调整、下单结算、模拟支付、取消订单、发货、确认收货、再次购买。
- 并发库存：订单创建使用事务和乐观锁扣减库存，提供并发扣库存验收脚本。
- AI 导购助手：悬浮聊天窗、商品搜索工具、订单查询工具、短期会话记忆、长期偏好记忆。
...
- Agent 不直接写商品、订单、用户等业务表，只通过后端 API 读取业务数据。
...
脚本会自动执行 `docker compose up -d`，显示容器状态，并打开前端页面。
...
管理员登录后可访问 `/admin`，普通用户访问会被重定向回首页。
...
- `JWT_SECRET` 用于后端签发 JWT，本地演示可用默认值，正式公开部署应替换。

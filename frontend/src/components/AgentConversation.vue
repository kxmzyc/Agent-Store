<template>
  <section class="agent-conversation" :class="`agent-conversation--${mode}`">
    ...
    <section v-if="agentTask" class="agent-task-card" :class="`is-${agentTask.stage}`" aria-label="当前导购任务">
      ...
      <div class="agent-task-controls">
        <label class="agent-preference-switch">
          <input v-model="ignorePreferences" type="checkbox" />
          <span>本次不使用长期偏好</span>
        </label>

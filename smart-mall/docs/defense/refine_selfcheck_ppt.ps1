...
function Add-ScoreRibbon {
    ...
    $label = $Slide.Shapes.AddTextbox(1, 80, 439, 88, 18)
    $label.Name = 'ScoreRibbonLabel'
    Set-ShapeText -Shape $label -Text '六项评分覆盖' -FontSize 10.5
    ...
    for ($i = 0; $i -lt $Labels.Count; $i++) {
        $chip = $Slide.Shapes.AddShape(5, $startX + (($width + $gap) * $i), 434, $width, 30)
        ...
        Set-ShapeText -Shape $chip -Text ([string] $Labels[$i]) -FontSize 9.5

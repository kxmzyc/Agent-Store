...
public class CouponService {
  ...
  @Transactional
  public boolean resetClaimEligibility(Long userId, Long couponId) {
    Coupon coupon = coupons.findById(couponId).orElseThrow(() -> BizException.notFound("优惠券不存在"));
    List<UserCoupon> claimed = userCoupons.findByUserIdAndCouponId(userId, couponId);
    if (claimed.isEmpty()) return false;
    userCoupons.deleteAll(claimed);
    int total = coupon.totalCount == null ? Integer.MAX_VALUE : coupon.totalCount;
    int remaining = coupon.remainCount == null ? 0 : coupon.remainCount;
    coupon.remainCount = Math.min(total, remaining + claimed.size());
    coupons.save(coupon);
    return true;
  }

<template>
  <section class="page admin-page">
    ...
    <template v-if="activeTab === 'couponAccess'">
      <section class="panel coupon-access-panel">
        <div class="panel-head">
          <div>
            <h2>优惠券领取权限</h2>
            <p>重置后，用户可以重新领取所选优惠券；已使用和已过期优惠券也支持重置。</p>
          </div>
        </div>
        <div class="admin-filter-bar">
          <input v-model="couponAccess.keyword" placeholder="用户名或手机号" @keyup.enter="searchCouponUsers" />
          <button type="button" @click="searchCouponUsers">搜索用户</button>
        </div>
        <div v-if="couponUsers.length" class="coupon-access-users">
          <button
            v-for="user in couponUsers"
            :key="user.id"
            class="coupon-access-user"
            :class="{ active: couponAccess.userId === user.id }"
            type="button"
            @click="couponAccess.userId = user.id"
          >
            <strong>{{ user.username }}</strong><span>ID {{ user.id }}{{ user.phone ? ` · ${user.phone}` : '' }}</span>
          </button>
        </div>
        <p v-else-if="couponAccess.searched" class="muted">没有找到匹配用户</p>
        <div class="coupon-access-actions">
          <select v-model="couponAccess.couponId" aria-label="选择优惠券">
            <option :value="null" disabled>选择优惠券</option>
            <option v-for="coupon in adminCoupons" :key="coupon.id" :value="coupon.id">
              {{ coupon.name }} · 剩余 {{ coupon.remainCount }}
            </option>
          </select>
          <button class="primary" type="button" :disabled="!couponAccess.userId || !couponAccess.couponId || couponAccess.resetting" @click="resetCouponEligibility">
            {{ couponAccess.resetting ? '处理中…' : '刷新领取资格' }}
          </button>
        </div>
      </section>
    </template>
...
const tabs = [
  ...
  { value: 'couponAccess', label: '优惠券权限' },
  { value: 'tags', label: '标签' },
  { value: 'logs', label: '日志' },
...
const couponAccess = reactive({ keyword: '', userId: null, couponId: null, searched: false, resetting: false })
...
watch(activeTab, async (tab) => {
  ...
  if (tab === 'couponAccess' && !adminCoupons.value.length) await loadCouponAccess()
...
async function loadCouponAccess() {
  const { data } = await api.get('/admin/coupons')
  adminCoupons.value = data || []
}

async function searchCouponUsers() {
  try {
    const { data } = await api.get('/admin/users', { params: { keyword: couponAccess.keyword } })
    couponUsers.value = data || []
    couponAccess.userId = couponUsers.value.some(user => user.id === couponAccess.userId) ? couponAccess.userId : null
    couponAccess.searched = true
  } catch (e) {
    toast.show(errorMessage(e))
  }
}

async function resetCouponEligibility() {
  if (!couponAccess.userId || !couponAccess.couponId) return
  if (!window.confirm('确认重置该用户对此优惠券的领取资格？')) return
  couponAccess.resetting = true
  try {
    await api.put(`/admin/users/${couponAccess.userId}/coupons/${couponAccess.couponId}/claim-eligibility`)
    toast.show('已刷新用户的优惠券领取资格')
    await loadCouponAccess()
  } catch (e) {
    toast.show(errorMessage(e))
  } finally {
    couponAccess.resetting = false
  }
}

...
def required_env(name: str) -> str:
    ...
    if not value or not value.strip() or value == LLM_PLACEHOLDER:
        raise RuntimeError(f"{name} must be configured")
    return value


DATABASE_URL = required_env("DATABASE_URL")
INTERNAL_SERVICE_SECRET = required_env("INTERNAL_SERVICE_SECRET")
...
app = FastAPI(title="Agent-Store Agent Service", lifespan=lifespan, default_response_class=Utf8JSONResponse)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def _build_ui_metadata(message: str, tools_used: list[str], model_used: bool, llm_requested: bool) -> ChatUi:
...
def extract_preferences(
    ...
    if rule_tags:
        return rule_tags
    if not use_llm or not has_preference_signal(message):
        return []
...
def handle_chat(
    ...
    try:
        ...
        if is_confirmation(message) and pending_key in pending_cart_actions:
            ...
            if request.useLlm and llm_enabled():
                try:
        ...
        new_tags = [] if request.ignorePreferences else extract_preferences(
            user_id, history, message, reply, use_llm=request.useLlm, callbacks=[preference_timing]
        )
        ...
        return ChatResult(
            ...
            timing=timing,
            ui=_build_ui_metadata(message, tools_used, model_used, request.useLlm),
        )
    finally:
